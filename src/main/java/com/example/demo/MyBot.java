...
//                break;
//            case "/news":
//                sendCryptoNewsOptions(update.getMessage().getChatId().toString());
...
public class MyBot extends TelegramLongPollingBot {
    ...
    private void sendCryptoNewsOptions(String chatId) {
        InlineKeyboardMarkup markupInline = new InlineKeyboardMarkup();
        List<List<InlineKeyboardButton>> rowsInline = new ArrayList<>();

        List<InlineKeyboardButton> rowInline = new ArrayList<>();

        InlineKeyboardButton bitcoinButton = new InlineKeyboardButton();
        bitcoinButton.setText("Bitcoin News");
        bitcoinButton.setCallbackData("news_bitcoin");
        rowInline.add(bitcoinButton);

        InlineKeyboardButton ethereumButton = new InlineKeyboardButton();
        ethereumButton.setText("Ethereum News");
        ethereumButton.setCallbackData("news_ethereum");
        rowInline.add(ethereumButton);

        // Continue adding more buttons as needed in the same manner
        ...
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText("Select the type of cryptocurrency news:");
    ...
    private void handleCallbackQuery(CallbackQuery callbackQuery) {
        ...
        if (callbackData.startsWith("news_")) {
            String cryptoType = callbackData.substring(5);  // Remove "news_" prefix to get the cryptocurrency type
            fetchAndSendCryptoNews(chatId, cryptoType);  // Pass the cryptocurrency type to the method
        } else if (callbackData.equals("trending")) {
            String trendingTokens = "1. Bitcoin\n2. Ethereum\n3. Binance Coin\n4. Solana\n5. Cardano";
            SendMessage sendMessage = new SendMessage(chatId, "Trending Tokens:\n" + trendingTokens);
            try {
                execute(sendMessage);
        ...
        // Other callback query handling as before
    }

    private void fetchAndSendCryptoNews(String chatId, String cryptoType) {
        final String apiKey = "3f2d665591714685b539e60a49892fad"; // Use your actual NewsAPI key
        final String query = cryptoType.equals("bitcoin") ? "bitcoin" : cryptoType.equals("ethereum") ? "ethereum" : "";
        final String url = "https://newsapi.org/v2/everything?q=" + query + "&sortBy=publishedAt&apiKey=" + apiKey;
        RestTemplate restTemplate = new RestTemplate();

        try {
            NewsResponse response = restTemplate.getForObject(url, NewsResponse.class);
            if (response != null && response.getArticles() != null && !response.getArticles().isEmpty()) {
                StringBuilder newsMessage = new StringBuilder("Latest " + cryptoType.substring(0, 1).toUpperCase() + cryptoType.substring(1) + " News:\n");
                for (Article article : response.getArticles().subList(0, Math.min(response.getArticles().size(), 5))) {
                    newsMessage.append("\n").append(article.getTitle()).append("\nRead more: ").append(article.getUrl()).append("\n");
                }
                SendMessage message = new SendMessage();
                message.setChatId(chatId);
                ...
                SendMessage message = new SendMessage();
                message.setChatId(chatId);
                message.setText("Sorry, I couldn't fetch the latest " + cryptoType + " news at this moment.");
                execute(message);
            }
        } catch (Exception e) {
            e.printStackTrace();
            SendMessage message = new SendMessage();
            message.setChatId(chatId);
            message.setText("An error occurred while fetching news for " + cryptoType + ".");
            try {
                execute(message);
            } catch (TelegramApiException telegramApiException) {
                telegramApiException.printStackTrace();
